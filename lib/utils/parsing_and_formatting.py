...
AQL_comparison_operators = {*AQL_single_value_comparison_operators, *AQL_one_to_many_value_comparison_operators}
...
def field_value_formatting(parsed_filter, stat_meta, idx):
        ...
        if stat_meta.get('type') == 'number':
            ...
            if stat_meta.get('maximum'):
                if value > float(stat_meta['maximum']):
                    format_warnings.append(f"provided value '{value}' at position {pos_idx} is greater than maximum "
                                     f"value '{stat_meta['maximum']}' for field '{field}'.")
            if stat_meta.get('minimum'):
                if value < float(stat_meta['minimum']):
                    format_warnings.append(f"provided value '{value}' at position {pos_idx} is less than minimum "
            ...
            if stat_meta.get('max-len'):
                if len(value) > stat_meta['max-len']:
                    format_warnings.append(f"provided value '{value}' at position {pos_idx} is longer than "
            ...
            if stat_meta.get('enum'):
                if value not in stat_meta['enum']:
                    format_warnings.append(f"{value} at position {pos_idx} is not a valid input for filter condition on metadat field '{field}' "
    ...
    if comp_op in AQL_single_value_comparison_operators:
        if len(validated_values) > 1:
            raise ValueError(f"Provided comparison operator '{comp_op}' expects 1 value, {len(validated_values)} values were provided.")
...
def parse_comparison_operator(comp_op, idx):
    if not comp_op:
        raise ValueError(f"please provide a comparison operator input for filter condition at position {idx}")
    # remove any trailing and leading whitespace
    comp_op = str(comp_op).strip().upper()
    if comp_op not in AQL_comparison_operators:
        raise ValueError(f"Input comparison operator in filter condition {idx} must be one of: " + ", ".join(["\'" + str(term) + "\'" for term in AQL_comparison_operators]))
    return comp_op
...
def parse_logical_operator(logic_op, idx, num_filters):
    if idx+1 >= num_filters:
        return None
    if not logic_op:
        raise ValueError(f"please provide a logical operator input for filter condition at position {idx}")
    # remove any trailing and leading whitespace
